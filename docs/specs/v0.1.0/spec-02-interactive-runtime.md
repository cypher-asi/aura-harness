...
> **Architectural Note (Orchestration Unification)**
>
> This spec originally described a `TurnProcessor` as the core orchestration
> loop. During implementation the architecture converged to a **single-flow
> design** where **`AgentLoop`** is the sole orchestrator. `TurnProcessor` has
> been removed. All references to `TurnProcessor` below should be read as
> `AgentLoop` — the multi-step loop that calls the model provider, dispatches
> tool execution through `KernelToolExecutor` (with parallel execution,
> per-tool timeouts, and policy enforcement), and manages streaming, budget,
> compaction, and cancellation. The `ModelCallDelegate` trait has also been
> removed; `AgentLoop` calls the provider directly.
>
> Key differences from the original spec:
> - **Single orchestrator:** `AgentLoop` (not `TurnProcessor`).
> - **Tool pipeline:** `KernelToolExecutor` → `ExecutorRouter` → `decode_tool_effect`.
> - **Streaming:** `TurnEvent` (unified event type) with `StreamReset` for fallback determinism.
> - **Store path:** Canonical path is `{data_dir}/db` everywhere (legacy `store/` auto-migrated).
...
│  ┌──────────────────────────────────────────────────────────────────┐  │
│  │                    AgentLoop (aura-agent)                         │  │
│  │                                                                   │  │
│  │   loop {                                                          │  │
│  │     1. Build request (compaction, budget, exploration)            │  │
│  │     2. Call ModelProvider.complete() (streaming or non-streaming) │  │
│  │     3. Accumulate response, emit TurnEvents                      │  │
│  │     4. If tool_use: cache → block-detect → execute via           │  │
│  │        KernelToolExecutor → auto-build check                     │  │
...
├─ aura-kernel            # UPDATED: policy, executor router, context
│  ├─ lib.rs
│  ├─ kernel.rs           # Existing single-step
│  ├─ policy.rs           # Existing
│  └─ context.rs          # Existing
├─ aura-agent             # NEW: AgentLoop orchestration
│  ├─ agent_loop/         # Multi-step orchestrator (sole flow)
│  ├─ kernel_executor.rs  # KernelToolExecutor (parallel, timeout, policy)
│  ├─ events.rs           # TurnEvent (unified streaming events)
│  └─ types.rs            # AgentToolExecutor trait, ToolCallInfo, etc.
...
## 7) AgentLoop Orchestration

> **Note:** This section originally described `TurnProcessor`. The implementation
> uses `AgentLoop` (in `aura-agent`) as the sole orchestrator. The loop semantics
> are equivalent; see the architectural note at the top of this document.

### 7.1 Configuration

```rust
// aura-agent/src/agent_loop/mod.rs (AgentLoopConfig)
...
### 7.2 AgentLoop (formerly Turn Processor Loop)

```rust
pub struct TurnProcessor<P: ModelProvider, S: Store> {  // Now: AgentLoop
    provider: Arc<P>,
    store: Arc<S>,
...
AURA_ANTHROPIC_MODEL=claude-opus-4-6
...
### Phase 4: AgentLoop Orchestration
- [x] Create `AgentLoop` struct (replaced original `TurnProcessor` design)
- [x] Implement multi-step conversation loop with streaming
- [x] Add tool execution via `KernelToolExecutor` (parallel, timeout, policy)
- [x] Add blocking detection, stall detection, budget management, compaction
...
### Phase 7: Integration
- [x] Wire AgentLoop into worker loop (CLI, TUI, node, WS, automaton)
- [x] Update swarm to use new provider system
- [x] Deprecate TS gateway usage

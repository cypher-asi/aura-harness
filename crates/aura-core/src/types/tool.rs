...
#[cfg(test)]
mod integration_match_tests {
    use super::*;

    fn make_install(integration_id: &str, provider: &str, kind: &str) -> InstalledIntegrationDefinition {
    ...
    fn provider_only_requirement_matches_when_provider_matches() {
        ...
        assert!(integration_match(&req(None, Some("github"), None), &candidate));
        assert!(!integration_match(&req(None, Some("gitlab"), None), &candidate));
    }
    ...
    fn kind_only_requirement_matches_when_kind_matches() {
        ...
        assert!(!integration_match(&req(None, None, Some("issue")), &candidate));
    }

    #[test]
    fn integration_id_only_requirement_matches_when_id_matches() {
        let candidate = make_install("id-1", "github", "git");
        assert!(integration_match(&req(Some("id-1"), None, None), &candidate));
        assert!(!integration_match(&req(Some("id-2"), None, None), &candidate));
    }

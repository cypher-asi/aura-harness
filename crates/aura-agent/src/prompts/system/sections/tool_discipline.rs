...
- Prior write_file / edit_file tool_use blocks shown in conversation history are redacted: their `content`, `old_text`, and `new_text` fields may contain `<<<AURA_ELIDED_*::N_bytes>>>` placeholders so the transcript fits in context. Never copy these placeholders verbatim into a new tool call; always supply the real content/edit you want applied. The resolver rejects elided sentinels with an InvalidArguments error.
";

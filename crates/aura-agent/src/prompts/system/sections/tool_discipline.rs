...
//! runtime (Phase 1's 32000-byte `write_file` chunk guard, Phase 2a's
...
- write_file must stay at or under 32000 bytes per call. If the file will be larger, create only the module doc + imports + one stub in your first write_file, then use edit_file with append_after_eof for the rest.
...
- Prior write_file / edit_file tool_use blocks shown in conversation history may have bulky `content`, `old_text`, and `new_text` fields removed and replaced with `_redacted` metadata so the transcript fits in context. Always supply the real content/edit you want applied.
";

...
pub(crate) async fn complete_with_retry(
    ...
    let mut last_error: Option<ReasonerError> = None;
    ...
    for (model_idx, model) in models.iter().enumerate() {
        ...
        for attempt in 0..=config.max_retries_per_model {
            ...
            match provider.complete(request.clone()).await {
                ...
                Err(e) => {
                    if matches!(e, ReasonerError::InsufficientCredits(_)) {
                        return Err(e);
                    }

                    let is_retryable = matches!(e, ReasonerError::RateLimited(_));
...
mod tests {
    ...
    fn classify_test_error(err: String) -> ReasonerError {
        if err.contains("402") {
            ReasonerError::InsufficientCredits(err)
        } else if err.contains("429") || err.contains("529") {
            ReasonerError::RateLimited(err)
        } else {
            ReasonerError::Internal(err)
        }
    }

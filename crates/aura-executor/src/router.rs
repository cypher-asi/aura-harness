...
mod tests {
    ...
    impl Executor for SelectiveExecutor {
        async fn execute(&self, _ctx: &ExecuteContext, action: &Action) -> Result<Effect, crate::ExecutorError> {
            if self.should_fail {
                return Err(crate::ExecutorError::ExecutionFailed("Executor configured to fail".into()));
            }
    ...
    async fn test_router_first_matching_executor_wins() {
        ...
        impl Executor for FirstExecutor {
            async fn execute(
                ...
                _ctx: &ExecuteContext,
                action: &Action,
            ) -> Result<Effect, crate::ExecutorError> {
    ...
    async fn test_executor_anyhow_error_produces_failed_effect() {
        ...
        impl Executor for AnyhowFailExecutor {
            async fn execute(
                ...
                Err(crate::ExecutorError::ExecutionFailed("database connection lost".into()))
            }
            fn can_handle(&self, _action: &Action) -> bool {
                true
            }
